...
function addProduct() {
    ...
    fetch('http://192.168.172.130:8080/products/add', {
        method: 'POST',
        headers: {
...
function fetchProducts(page) {
    fetch(`http://192.168.172.130:8080/products/get/${categoryId}?page=${page}`, { // Sửa đường dẫn URL ở đây
        method: 'GET',
        headers: {
             'Authorization': 'Bearer ' + token,
...
function deleteRow(id){
   ...
   if(confirm("bạn chắc chắn xóa không")){
       fetch(`http://192.168.172.130:8080/products/${id}`, {
        method: 'DELETE',
        headers: {
             'Authorization': 'Bearer ' + token,
            'Content-Type': 'application/json'
...
function sendUpdateRequest(formData, productid) {
    ...
    fetch(`http://192.168.172.130:8080/products/${productid}`, {
        method: 'PUT',
        headers: {
            'Authorization': 'Bearer ' + token,
        },
